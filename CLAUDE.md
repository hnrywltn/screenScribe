...
- **Email provider: Resend**, called via raw `fetch` to their REST API (`worker/lib/email.ts`) — no SDK dependency needed for one endpoint. **No real API key exists** — `RESEND_API_KEY` is unset, so it logs `[email] ... would have emailed ...` instead of sending. Needs a real Resend account + verified sender before emails actually go out.
...
- **A real Resend API key.** `worker/lib/email.ts` is wired up and called at the right point, but `RESEND_API_KEY` is unset, so it logs instead of sending.
