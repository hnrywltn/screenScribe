...
This is the core of what ScreenScribe is supposed to do. One piece works, the rest doesn't:
...
- **Scene/slide detection** — approach undecided (frame diffing vs. a dedicated scene-detection library). Will also shell out to ffmpeg (frame extraction), likely living alongside `transcodeToMp4()` in `worker/lib/ffmpeg.ts`.
