...
Email + password. `lib/auth.ts`: `bcryptjs` for hashing (12 rounds), `jose` for a signed HS256 JWT in an httpOnly cookie (`SESSION_SECRET` in `.env.local`, 30-day expiry, `purpose: "session"` claim checked explicitly on the way out). `app/api/auth/{signup,login,logout}/route.ts` handle the three actions; `LoginForm`/`SignupForm` client components post to them. Verified end-to-end with a real browser session (signup → dashboard → logout → gated again, duplicate-email and wrong-password error paths, already-authenticated redirect away from `/login`/`/signup`). See `CLAUDE.md` → "Decided: auth mechanism" for what's still missing (password reset, rate limiting — email verification is no longer on that list, see below).

**Form inputs are uncontrolled (`defaultValue` + `FormData` at submit), not `useState`-controlled** — browser autofill doesn't reliably fire the events a controlled input needs to stay in sync with React state, a known cross-browser issue. See `CLAUDE.md` → "Decided: auth mechanism" for the full explanation and how it was verified (simulated the actual autofill failure mode, not just normal typing).

## Email verification — built, non-blocking

Signup logs the user in immediately regardless of verification status — a banner (`components/VerifyEmailBanner.tsx`, rendered from `app/(app)/layout.tsx`) is the only enforcement, not a login block. Verification tokens are short-lived JWTs signed with the same `SESSION_SECRET` sessions use (`lib/auth.ts` → `createVerificationToken`/`verifyVerificationToken`) — no separate token table, the signature + a `purpose: "email-verification"` claim + 24h expiry is the whole mechanism. That shared secret is exactly why the `purpose` claim is checked on *both* token types now — without it, a verification link could double as a forged session. `lib/email.ts` (root package, separate from `worker/lib/email.ts`) sends via the same raw-`fetch`-to-Resend pattern. Verified end-to-end with a real browser and a real Resend send (not a mock) — see `CLAUDE.md` → "Decided: email verification" for the full account, including a Next.js gotcha it surfaced: route folders starting with `_` are private and don't route at all.
