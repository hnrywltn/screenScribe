...
- **Transcription** — direction and model size both decided: local **whisper.cpp** (no cloud API, no exceptions), **`medium` model, quantized** (must be — the unquantized FP16 build runs slower than real-time on CPU per benchmark data, the quantized build doesn't). Chosen after actually costing it out against Railway's compute pricing rather than guessing — see `CLAUDE.md` → "Decided: transcription" for the numbers. **No invocation code written yet, no model file or binary installed** — decision only.
