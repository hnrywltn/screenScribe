...
- `users` — `id` (UUID PK), `email` (unique), `password_hash`, `created_at`, `email_verified_at` (nullable, `NULL` = unverified), `phone` (nullable, **optional**, no format validation beyond a loose 7–15-digit sanity check — see `CLAUDE.md` → "Decided: email verification" area for why it's optional). Written to by `app/api/auth/signup/route.ts`, read by `app/api/auth/login/route.ts`, by the worker (to find the address for the "ready" email), and by `app/(app)/layout.tsx` (whether to show the verify-email banner) — email+password auth is built, see `CLAUDE.md` → "Decided: auth mechanism" and "Decided: email verification". Nothing currently reads `phone` — captured for future use, not acted on.
